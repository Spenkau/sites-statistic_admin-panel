<x-app-layout>
    ...
    @section('script')
        <script>
            ...
            addCollaboratorsForm.addEventListener('click', (event) => {
                if (event.target.matches('.remove-collaborator-btn')) {
                    const btn = event.target;
                    const input = btn.previousElementSibling
            ...
            const getAddUsersMarkup = (user) => {
                ...
                <li class="list-group-item d-flex text-center gap-3">
                    ...
                    <input type="hidden" data-id="${user.id}" data-name="${user.name}" />
                    <button type="button" class="remove-collaborator-btn btn btn-close"></button>
                </li>
            `
            }
        </script>
